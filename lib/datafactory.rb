module DataFactory
	...
	module DataFactory::Mono
		...
		def self.get_client_info(env)
			if DataFactory::MOCK_DATA_FOR.include?(env) then
		...
		def self.get_statements(env, selected_account, date_start = Time.now.to_i - 30*24*60*60, date_end = Time.now.to_i)
			if DataFactory::MOCK_DATA_FOR.include?(env) then
		...
		def self.return_statements(env, selected_account)
			statements = self.get_statements(env, selected_account)
			parsed_statements = []
	...
	module DataFactory::ETH
		...
		BALANCE_PARAMS = {module: 'account',action: 'balancemulti',tag: 'latest',address: DataFactory::ETH::ETH_ADDRESSES}
		MOCK_DATA = {
		...
		def self.get_client_info(env)
			...
			puts last_price.to_s
			puts balance.to_s
			return last_price.merge(balance[0])
		end

		def self.return_client_info(env)
			client_info = self.get_client_info(env)
			accounts = []
			in_float = client_info['balance'].to_i/1000000000000000000.to_f
			bal_eth = in_float.round(4)
			bal_usd = bal_eth * client_info['ethusd'].to_f
			bal_usd = bal_usd.round(0)
			account = {
				currencyCode: 'ETH',
				type: 'ETH',
				maskedPan: "#{ETH_ADDRESSES[0..4]}..#{ETH_ADDRESSES[-5..-1]}",
				balance: bal_eth,
				balance_usd: bal_usd,
				id: ETH_ADDRESSES,
				maskedPan_full: ETH_ADDRESSES
			}
			return account
		end

		def self.get_statements(env, address)
			if MOCK_DATA_FOR.include?(env)
		...
		def self.return_statements(env, address)
			statements = self.get_statements(env, address)
			parsed_statements = []
			statements.each do |stat| 
