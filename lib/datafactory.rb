	...
	module DataFactory::ETH
		...
		def self.get_client_info()
			if MOCK_DATA_FOR.include?(ENVIRONMENT)
				...
				last_price = DataFactory.send_request(ETH_API_URL, '', LAST_PRICE_PARAMS, ETH_TOKEN)
				balance = DataFactory.send_request(ETH_API_URL, '', BALANCE_PARAMS, ETH_TOKEN)
			end
			info = {
		...
		def self.parse_client_info(client_info)
			accounts = []
		...
		def self.return_client_info()
			if ! (ETH_ADDRESSES.nil? || ETH_ADDRESSES.empty?) then
				client_info = self.get_client_info()
				accounts = self.parse_client_info(client_info)
			else
				accounts = [] 
			end
			return accounts
		...
		def self.get_statements(address)
			if MOCK_DATA_FOR.include?(ENVIRONMENT)
				...
				statements = DataFactory.send_request(ETH_API_URL, '', params, ETH_TOKEN)
			end
			return statements
		end

		def self.parse_statements(statements, address)
			result = []
			statements.each do |stat| 
				...
				a = {
					...
					etherscanUrl: "#{ETH_TX_URL}#{stat['hash']}",
					txFee: fee,
				}
				result.push(a)
			end
		...
		def self.return_statements(address)
			...
			parsed_statements = self.parse_statements(statements, address)
			return parsed_statements
		end
	end
